...
public class TimeDependentSignalGenerator extends SingleNoteNoiseSignalGenerator {
    public TimeDependentSignalGenerator(int samplingFrequency, double frameDuration, double noiseLevel) {
        this.samplingFrequency = samplingFrequency;
        this.frameDuration = frameDuration;
        this.noiseLevel = noiseLevel;
    }

    public TimeDependentSignalGenerator() {
        this(44100, 0.02, 0.1);
    }
    ...
    public String generateRandomSignal() {
        ...
        List<Double> timeDepemdencies = new ArrayList<>(harmonicCount);
        for (int i = 0; i < harmonicCount; i++) {
            amplitudes.add(ThreadLocalRandom.current().nextDouble());
            initialPhases.add(ThreadLocalRandom.current().nextDouble() * 2 * Math.PI);
            timeDepemdencies.add(ThreadLocalRandom.current().nextDouble() * 0.2 - 0.1);
        }
        ...
        for (int i = 0; i < signalSize; i++) {
            ...
            for (int j = 0; j < harmonicCount; j++) {
                signalValue += (amplitudes.get(j) * (1 + timeDepemdencies.get(j) * i / signalSize)) * Math.sin(2 * Math.PI * currentTime * toneFrequency * (j + 1) + initialPhases.get(j));
            }
